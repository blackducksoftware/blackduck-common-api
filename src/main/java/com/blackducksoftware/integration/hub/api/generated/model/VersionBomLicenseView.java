package com.blackducksoftware.integration.hub.api.generated.model;

import com.blackducksoftware.integration.hub.api.HubModel;
import com.blackducksoftware.integration.hub.api.generated.enumeration.VersionBomLicenseEnum;
import com.blackducksoftware.integration.hub.api.generated.model.VersionBomLicenseView;

//this file should not be edited - if changes are necessary, the generator should be updated, then this file should be re-created
public class VersionBomLicenseView extends HubModel {
    public String license;
    public String licenseDisplay;
    public VersionBomLicenseEnum licenseType;
    public java.util.List<VersionBomLicenseView> licenses;

}
